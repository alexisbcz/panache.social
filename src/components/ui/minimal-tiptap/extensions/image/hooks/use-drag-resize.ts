...
/**
 * React hook for managing resizable element dimensions with drag and keyboard interactions.
 *
 * Supports resizing from left or right edges, grid snapping, aspect ratio preservation, and min/max constraints. Allows cancellation via Escape key and notifies on dimension changes.
 *
 * @returns An object containing:
 *   - `initiateResize`: Function to start resizing from a specified direction.
 *   - `isResizing`: Boolean indicating if resizing is active.
 *   - `updateDimensions`: Function to set dimensions externally.
 *   - `currentWidth`: The current constrained width.
 *   - `currentHeight`: The current constrained height.
 */
